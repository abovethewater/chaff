  ...
  Step (
    function () {
      ...
      snippets.forEach(function (snippet) {
        readGitFile(version, "articles", snippet.filename, group());
      });
    },
    function (err, files) {
...
function readGitDir(version, dir, callback) {
    Step(
      function readDir() {
...
function readGitFile(version, directory, filename, callback) {
    Step(
      function readFile() {
        Git.readFile(version, directory + "/" + filename, this);   
      },
      callback
    );    
}
...
var Data = module.exports = {
  ...
  snippet: Git.safe(function snippet(version, path, callback) {
    ...
    Step(
      function () {
        ...
        readGitFile(version, "articles", filename, this);
      },
      function (err, code) {
  ...
  article: Git.safe(function article(version, name, callback) {
    ...
    Step(
      function getArticleMarkdown() {
        readGitFile(version, "articles", name + "." + nconf.get("markdownSuffix"), this);
      },
  ...
  fullArticle: Git.safe(function fullArticle(version, name, callback) {
    ...
    Step(
      ...
      function loadExtras(err, props) {
        ...
        Git.log("articles/" + name + "." + nconf.get("markdownSuffix"), this.parallel());
  ...
  author: Git.safe(function author(version, name, callback) {
    Step(
      function getAuthorMarkdown() {
        ...
        readGitFile(version, "authors", name + "." + nconf.get("markdownSuffix"), this);
      },
  ...
  categories: Git.safe(function articles(version, callback) {
    Step(
      function getListOfArticles() {
        readGitDir(version, "articles", this);
      },
      function readArticles(err, results) {
        if (err) { callback(err); return; }
        var group = this.group();
        results.files.forEach(function onFile(filename) {
          var md = Tools.markdown(filename);
          if (md) {
            Data.article(version, md.name, group());
          }
        });
      },
  ...
  readDescription: Git.safe(function readDescription(version, callback) {

    Step(
      function getListOfFiles() {
        readGitDir(version, "./", this);
      },
      function findDescription(err, results) {
        if (err) { callback(err); return; }
        var suffix = nconf.get("markdownSuffix");
        var filenames = results.files.filter(function onFile(filename) {
          if (filename === "description." + suffix) {
            return true;
          }          
        });
        return filenames[0];
      },
      function readDescription(err, filename) {
        if (err) { callback(err); return; }
        readGitFile(version, ".", filename, this);
      },
      callback
    )
  })
